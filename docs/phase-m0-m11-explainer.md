...
| M12.1 | 冻结 strict goodput／work ledger／fairness 合同 | `m12-metric-contract-v1.1`；workload 与 attempts 分离 | ✅ 完成；三套 regime 只作 sensitivity |
...
M12.1 已执行：strict useful-token goodput 按固定 horizon 排名；workload truth 独立于 attempts，因此 zero-attempt drop 不能从 offered／fairness 分母消失。input／output goodput 分列，output 相对退化≤0.1%；所有 retry／abort GPU work 全计费，KVS work 必须进入 M12.2 completion scheduling。fairness 冻结为 tier attainment≥0.80、Jain≥0.90、相对 baseline 单 tier退化≤0.02。三套 regime 只用于 sensitivity，禁止解释为真实硬件。
