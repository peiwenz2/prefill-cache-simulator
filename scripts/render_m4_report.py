...
def main() -> int:
    ...
    stop_complex = max(
        float(s5["token_weighted_hit_rate"]),
        float(s6["token_weighted_hit_rate"]),
    ) <= float(s3["token_weighted_hit_rate"])
    stop_text = (
        "S5／S6 没有超过 S3，stop gate 触发"
        if stop_complex
...
<section id="eval"><h2>3. Sensitivity 与 mechanism checks</h2><h3>3.1 Stale view</h3>{stale_table}<p class="fine">Delay 按 mean request service time 约 488 normalized ms 取 0.1×／1×／10×。</p><h3>3.2 Capacity：fixed-total 与 fixed-per-node</h3>{capacity_table}<h3>3.3 S4 family cap／hot-block exclusion</h3>{s4_table}<h3>3.4 Seed replay</h3>{seed_table}<p class="fine">结果由 CSV 实时计算。loss_probability=0 时 S3／S4／S5 不消费随机 stream，zero variance 是 deterministic contract check，不当成鲁棒性证据。</p></section>
<section id="risk"><h2>4. 风险与边界</h2>{table(["风险", "当前状态", "下一步"], [["Normalized service time", "只能做相对排序，不能叫 production TTFT", "Phase B 用 GPU/KVT calibration"], ["S4 conversation proxy", "仅依赖 online prefix family，不是真 session id", "接真实 session selector 对照"], ["S3 overload", "N=8 request skew 3.23×", "保留为 ceiling，不作为默认生产策略"], ["Eviction regret", "当前是 re-access-after-eviction count", "增加 bounded regret window"]])}<div class="callout warn"><b>诚实回答</b>：这批结果验证的是 LOCAL_ONLY cache placement，不证明 KVS、SLO goodput 或 Decode lease 的收益。后者从 M5 开始单独建账。</div></section>
